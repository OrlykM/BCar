...
class CarViewFilterSet(generics.ListAPIView):
    ...
    filterset_fields = ['number_of_seats', 'category_type', 'body_type',
                        'fuel_type']
...
class CarViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in [
            'show_approve_status'] and self.request.user.is_staff == 1:
            permission_classes = (IsModerator,)
            return [permission() for permission in permission_classes]
        else:
            permission_classes = (IsOwner,)
...
class CarViewShowAll(viewsets.ModelViewSet):
    queryset = Car.objects.filter(is_approved=True, available_now=True)

    def show_all(self, request, *args, **kwargs):
        car_data = Car.objects.filter(is_approved=True,
                                      available_now=True).all()
        return Response(CarSerializer(car_data, many=True).data)

class CarGetOne(viewsets.ModelViewSet):
    queryset = Car.objects.filter(is_approved=True, available_now=True)

    def show(self, request, *args, **kwargs):
        car_data = Car.objects.filter(id=kwargs['car_id']).first()
        return Response(CarSerializer(car_data).data)